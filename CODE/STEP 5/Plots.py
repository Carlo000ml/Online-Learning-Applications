...
def plot_comparison_3(x,arr1,arr2,arr3,name1,name2,name3,yname):
    y1 = np.mean(arr1,axis=0)
    y2 = np.mean(arr2,axis=0)
    y3 = np.mean(arr3,axis=0)
    ...
    # Plot the line
    plt.plot(x, y3, label=name3)

    # Plot the upper confidence bound
    plt.fill_between(x, quantile_down3, quantile_up3, alpha=0.5, label='95% confidence bound')
